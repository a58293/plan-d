...
const ProductCard: React.FC<{ project: ProjectItem; index: number }> = ({ project, index }) => {
  const ref = useRef<HTMLDivElement>(null);
  const hasGallery = project.galleryImages && project.galleryImages.length > 0;

  return (
    <Link to={`/installation/${project.id}`} className="col-span-1 aspect-[3/2] relative group block">
      {hasGallery && (
        <>
      ...
      <motion.div
        ref={ref}
        className="relative h-full w-full cursor-pointer overflow-hidden bg-white transition-all duration-500"
    ...
    <section className="relative w-full min-h-screen bg-white flex flex-col">
      ...
      <div className="w-full px-4 py-8 md:px-12 md:py-12 space-y-8 md:space-y-10">
        <SectionPageIntro
          ...
          lines={[
            "以物为媒，建立清晰的品牌触点。",
            "从产品包装、文创载体到视觉延展，统一呈现可被识别、可被记忆的设计系统。",
            "让产品在陈列、传播与使用场景中保持稳定的视觉秩序。",
          ]}
        />
