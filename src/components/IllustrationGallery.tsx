    ...
    const run = async () => {
      const next = await Promise.all(
        illustrationCategories.map(async (group) => {
          const checks = await Promise.all(group.images.map(async (src) => ((await checkImageExists(src)) ? src : null)));
          return {
            ...group,
            images: checks.filter(Boolean) as string[],
          };
        })
      );
  ...
  return (
    <div className="pointer-events-none absolute inset-0 hidden lg:block">
      <svg className="absolute inset-0 h-full w-full" viewBox="0 0 100 100" preserveAspectRatio="none" aria-hidden="true">
        <line x1="37" y1="-4" x2="23" y2="104" stroke="rgba(255,255,255,0.8)" strokeWidth="0.18" />
        <line x1="69" y1="-4" x2="55" y2="104" stroke="rgba(255,255,255,0.8)" strokeWidth="0.18" />
      </svg>
    </div>
  );
}
          ...
          {images.length > 0 ? (
            ...
            <div className="rounded-[24px] border border-dashed border-gray-200 bg-[#FBFAF7] px-6 py-14 md:px-10 md:py-20 text-center">
              <p className="font-site text-[16px] md:text-[18px] leading-[1.9] tracking-[0.04em] text-[#6B7280]">
                <SplitColorText
                  text={`请在 public/images/illustration/${group.key}/ 中放入 01.jpg / 01.webp 这类顺序编号图片。`}
                  defaultColor="#6B7280"
                  fontClass="font-site"
                />
              </p>
