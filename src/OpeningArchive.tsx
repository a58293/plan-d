...
export default function OpeningArchive(){
 ...
 useEffect(()=>{if(!interactive)return;let timer=0;const wake=()=>{setToolsVisible(true);clearTimeout(timer);timer=window.setTimeout(()=>{if(!storyOpen)setToolsVisible(false);},3000);};wake();const el=overlay.current;el?.addEventListener('pointermove',wake);el?.addEventListener('pointerdown',wake);el?.addEventListener('keydown',wake);el?.addEventListener('focusin',wake);return()=>{clearTimeout(timer);el?.removeEventListener('pointermove',wake);el?.removeEventListener('pointerdown',wake);el?.removeEventListener('keydown',wake);el?.removeEventListener('focusin',wake);};},[interactive,storyOpen]);
 const stage=useRef<HTMLDivElement>(null);
 const canExplore=mode==='poster'||held||playError;
 const tilt=usePosterTilt(interactive&&canExplore,stage);
