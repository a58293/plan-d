...
export default function DesktopNavigation(){
 ...
 const renderItem=(item:NavigationItem)=>item.children?<button key={item.title} onMouseEnter={()=>delay(()=>setTrail(t=>[...t,item]))} onMouseLeave={cancel} onClick={()=>{cancel();setTrail(t=>[...t,item]);}}>{item.title}<span>›</span></button>:item.href?<a key={item.title} href={item.href} onClick={close}>{item.title}<span>↗</span></a>:<span key={item.title} className="archive-pending">{item.title}<small>资料待公开</small></span>;
