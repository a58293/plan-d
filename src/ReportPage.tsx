...
import contactCard from '../public/contact/xiaohongshu-card.jpg?url';

export default function ReportPage() {
  const [notice,setNotice]=useState('');
  async function copyAccount() {
    try {await navigator.clipboard.writeText('6765312465');setNotice('请打开小红书搜索该账号，再通过私信联系。');}
  ...
  return <div className="legal-world report-world">
    ...
    <main><p className="legal-kicker">原创保护 · 联系我们</p><h1>举报说明</h1><p className="legal-intro">如发现疑似仿冒、盗用图片或冒充官方的情况，请通过官方小红书账号私信提供线索。</p>
      ...
      <section className="report-social" aria-labelledby="social-title"><div><p className="legal-kicker">小红书 · 官方私信</p><h2 id="social-title">绘屿造物 Lumen Auralis</h2><p>小红书号：<strong className="social-account">6765312465</strong></p><p>复制账号后，在小红书中搜索；也可以保存名片，在小红书中扫描识别二维码，进入账号主页后发起私信。</p><div className="report-actions"><button type="button" onClick={copyAccount}>复制小红书号</button><a href={contactCard} download="绘屿造物-小红书名片.jpg">保存账号名片 ↓</a></div><p>请核对账号名称和小红书号。</p><p role="status">{notice}</p></div><a href={contactCard} target="_blank" rel="noopener noreferrer" aria-label="查看小红书账号名片原图"><img src={contactCard} alt="绘屿造物 Lumen Auralis 小红书名片，账号 6765312465，含账号二维码" width="987" height="1347"/></a></section>
    </main>
  </div>;
}
