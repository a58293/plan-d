...
export default function MobileNavigation(){
 ...
 return <div ref={root} className="mobile-navigation" data-open={open} style={position?{left:position.x,top:position.y,right:'auto',bottom:'auto'}:undefined}>
 {open&&<nav id="orb-menu" className="orb-menu" aria-label="分层目录" style={position?{right:'auto',bottom:'auto',left:Math.max(8,Math.min(innerWidth-296,position.x-232))-position.x,top:Math.max(8,Math.min(innerHeight-348,position.y-284))-position.y}:undefined} onKeyDown={e=>{e.stopPropagation();if(e.key==='Escape'){close();orb.current?.focus();}}}>
 ...
 onPointerUp={()=>{orbDrag.current=null;}} onPointerCancel={()=>{orbDrag.current=null;orbMoved.current=true;}}
