...
export default function Breadcrumbs({path, section}: {path: string; section?: string}) {
  ...
  if(path==='/stories/official'){items.push({label:'影像与故事',href:'/stories'},{label:'官方拍图',href:section?path:undefined});} else if(path==='/stories/openings'){items.push({label:'影像与故事',href:'/stories'},{label:'往期序章'});} else if(path==='/stories/collectors'){items.push({label:'影像与故事',href:'/stories'},{label:'藏家自拍',href:section?path:undefined});} else if(categoryPages[path]) {
